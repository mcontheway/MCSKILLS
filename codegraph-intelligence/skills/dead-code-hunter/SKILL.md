...
2. Narrow the scope: specify a module, directory or pattern (e.g. functions in `utils/` or classes under `services/`) to avoid scanning the entire codebase.
3. Use `codegraph_files` and `codegraph_search` to retrieve candidate symbols matching the scope.
4. For each candidate, call `codegraph_callers` to see if any functions or modules reference it. If the callers list is empty, mark the symbol as a potential dead code candidate.
5. Exclude symbols that are:
...
6. Compile a list of high-confidence and medium-confidence dead code:
...
7. Recommend verifying each candidate by:
...
8. Provide the list with file paths, symbol names and confidence levels.
